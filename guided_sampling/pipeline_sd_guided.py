...
#         timesteps = torch.cat([timesteps, torch.tensor([0], device=timesteps.device, dtype=timesteps.dtype)])
#         sigmas = torch.cat([sigmas, torch.tensor([0], device=sigmas.device, dtype=sigmas.dtype)])
        ...
        x = model_input * init_sigmas
        
        #import pdb; pdb.set_trace()
        for i, sig in enumerate(sigmas_short):
            ts = timesteps_short[i].unsqueeze(0)
            latent_input = x / ((sig.item() ** 2 + 1) ** 0.5)
            ...
            x_prev = self.scheduler.step(e_t, ts.item(), latent_input).prev_sample
    ...
    def node_solver_adaptive(self, 
        ...
        #import pdb; pdb.set_trace()
        solution = odeint(ODEFunc, model_input * init_sigmas, sigmas_short, method=solver_type, adjoint_params=model_input)
    ...
    def node_solver_adaptive_type2(self, 
        ...
        #index = len(self.scheduler.timesteps)-index
        #timesteps = torch.flip(self.scheduler.timesteps, dims=(0,))[index:]
        timesteps = self.scheduler.timesteps
        alphas_cumprod = self.scheduler.alphas_cumprod
                ...
                for j in range(repeat):
                    ...
                    with torch.enable_grad():
                        if is_grad[i]:
                            ...
                            # intermediate[0].save(os.path.join("./outputs/multimodal_dog_ad/", "int_{}.png".format(i)))
