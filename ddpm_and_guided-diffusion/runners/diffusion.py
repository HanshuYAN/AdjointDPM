...
from functions import get_optimizer, normalize_by_pnorm, clamp, clamp_by_pnorm
...
class Diffusion(object):
    ...
    def sample_image(self, x, model, last=True, classifier=None, base_samples=None):
        ...
        if self.args.sample_type == "generalized":
            ...
            # x = x.cpu()
        elif self.args.sample_type in ["neuralode"]:
            from dpm_solver.node_solver import NoiseScheduleVP, model_wrapper, NODE_Solver
            def model_fn(x, t, **model_kwargs):
                out = model(x, t, **model_kwargs)
                # If the model outputs both 'mean' and 'variance' (such as improved-DDPM and guided-diffusion),
                # We only use the 'mean' output for DPM-Solver, because DPM-Solver is based on diffusion ODEs.
    ...
    def attack(self):
        #define the guided generation model
        model = GuidedDiffusion_Model(
                    image_size=self.config.model.image_size,
                    in_channels=self.config.model.in_channels,
                    model_channels=self.config.model.model_channels,
        ...
        criteria = torch.nn.CrossEntropyLoss().to(self.device)

        #classification model
        classification_model = resnet50(pretrained=True)
        classification_model = classification_model.to(self.rank)
        ...
        for k in range(self.args.num_samples // self.args.attack_batch_size):
            x_T = torch.randn(
                self.args.attack_batch_size,
                config.data.channels,
                config.data.image_size,
                config.data.image_size,
                device=self.device,
            ...
            x, classes = self.sample_image(x_T, model, classifier=classifier,)
            x = inverse_data_transform(config, x)

            pred_class = classification_model(preprocess(x)).softmax(dim=-1).argmax(dim = -1)

            for i in range(self.args.attack_batch_size):
                if pred_class[i] == classes[i]:
                    total+=1
                fname_pre=os.path.join(self.args.image_folder, f"{k}_{i}_{classes[i]}_{pred_class[i]}_pre.png")
                out_img_pre = x[i]
                tvu.save_image(out_img_pre, fname_pre)

            for i in range(self.args.nb_iter):
                # NOTE: This means that we are producing each predicted x0, not x_{t-1} at timestep t.
                x, classes = self.sample_image(x_T, model, classifier=classifier,)
                #x = [inverse_data_transform(config, y) for y in x]
                x = inverse_data_transform(config, x)

                pred_before = classification_model(preprocess(x)).softmax(dim=-1)
                target = classes

                loss = criteria(pred_before, target)
                loss.backward()
                grad = x_T.grad.data
                grad = normalize_by_pnorm(grad)
                
                x_T.data = x_T.data + self.args.lr * grad 
                x_T.data = clamp(x_T.data, initial, self.args.clamp_min, self.args.clamp_max)
                x_T.grad.data.zero_()
                print('iter: {}, loss: {}'.format(i, loss))
            
            after_x, classes = self.sample_image(x_T, model, classifier=classifier,)
            after_x = inverse_data_transform(config, after_x)
            pred_after = classification_model(preprocess(x)).softmax(dim=-1).argmax(dim=-1)
            for i in range(self.args.attack_batch_size):
                if pred_class[i] == classes[i] and pred_class[i]!=pred_after[i]:
                    attack_num+=1
                fname_pre=os.path.join(self.args.image_folder, f"{k}_{i}_{pred_after[i]}_after.png")
                out_img_pre = after_x[i,:]
                tvu.save_image(out_img_pre, fname_pre)
